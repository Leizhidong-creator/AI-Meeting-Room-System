...
- **音频增强链路优化：** 针对设备噪声、空调底噪和远场收音导致的识别质量下降问题，部署 DeepFilterNet3模型，探索会议室声音降噪精转链路；并将降噪 SNR、CER 和 DER 作为固定会议样本的评测指标。
- **模型适配与转写优化：** 针对政企会议中的专业术语、口音和流式字幕抖动问题，主导 Paraformer Streaming 与 Qwen3-ASR 1.7B 双阶段转写链路，通过术语词典、稳定片段二次转写、CT-Punc 后处理和 Provider 降级机制提升文本可读性；Qwen3-ASR 1.7B 对 10 秒音频单次推理约 **0.32–0.40 秒**，大大降低了转写延迟。
